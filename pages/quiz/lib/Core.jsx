...
import { setDoc, getFirestore, getDocs, query, doc, getDoc } from "firebase/firestore";
import {
  auth
} from "../../../lib/firebase";

const Core = function ({
  questions, appLocale, showDefaultResult, onComplete, customResultPage, onQuestionSubmit, 
   continueTillCorrect, revealAnswerOnSubmit, allowNavigation,
}) {
  ...
  useEffect(() => { //to update points for user 
    if (endQuiz) {
  ...
  const renderAnswerInResult = (question, userInputIndex) => {
    const { answers, correctAnswer, questionType } = question;
      return (
        <div>
           Answer : {answers[correctAnswer-1]}
        </div>
      );
  };
  ...
  const renderQuizResultQuestions = useCallback(() => {
    ...
    return (filteredQuestions || questions).map((question, index) => {
      ...
      return (
        <div  key={index + 1}>
          <div className="font-black text-left mb-3 ">
